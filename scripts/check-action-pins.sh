...
#
# Each owner beyond `actions` is here because this repo genuinely runs their
# code, and each was added deliberately:
#   astral-sh    setup-uv, the toolchain every Python job is built on
#   docker       buildx/qemu/login/metadata/build-push, the image pipeline
#   peter-evans  dockerhub-description, syncs README.md to the Hub overview
#   pypa         gh-action-pypi-publish, the Trusted Publishing (OIDC) upload
